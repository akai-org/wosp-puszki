 ...
 * @author kabix09
 *
 * Class counting money from @see confirmed charity boxes
 */
class ConfirmedMoneyCounter extends MoneyCounter
{
    ...
    protected static function init(): array
    {
        ...
        return array_reduce(CurrencyEnum::values(), function ($prevIterResult, $item) {
            ...
            return array_merge(
                $prevIterResult,
                [
                    $item => new Money((int)CharityBox::where('is_confirmed', '=', 1)->sum($currentColumnName) * 100,
